...
["compress", "ie8", "mangle", "sourceMap", "toplevel", "wrap"].forEach(function (name) {
    if (name in program) {
        options[name] = program[name];
    }
});
if (program.beautify) {
    options.output = _typeof(program.beautify) == "object" ? program.beautify : {};
    if (!("beautify" in options.output)) {
        options.output.beautify = true;
    }
}
if (program.comments) {
    if (_typeof(options.output) != "object") options.output = {};
    options.output.comments = typeof program.comments == "string" ? program.comments : "some";
}
if (program.define) {
    if (_typeof(options.compress) != "object") options.compress = {};
    if (_typeof(options.compress.global_defs) != "object") options.compress.global_defs = {};
...
if (program.mangleProps) {
    if (program.mangleProps.domprops) {
        ...
        if (_typeof(program.mangleProps) != "object") program.mangleProps = {};
        if (!Array.isArray(program.mangleProps.reserved)) program.mangleProps.reserved = [];
        require("../tools/domprops").forEach(function (name) {
            UglifyJS._push_uniq(program.mangleProps.reserved, name);
        });
    }
    if (_typeof(options.mangle) != "object") options.mangle = {};
...
if (program.nameCache) {
    ...
    if (options.mangle) {
        ...
        if (options.mangle.properties) {
            if (_typeof(options.mangle.properties) != "object") options.mangle.properties = {};
...
var convert_path = function convert_path(name) {
    return name;
};
if (_typeof(program.sourceMap) == "object" && "base" in program.sourceMap) {
